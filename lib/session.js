...
export function createUserSession(res, { discordId, discordUsername, discordAvatar }) {
  const token = sign({ discordId, discordUsername, discordAvatar: discordAvatar || null, exp: Date.now() + MAX_AGE_SECONDS * 1000 });
...
const RETURN_COOKIE_PREFIX = "everix_oauth_return_";

function isSafeReturnPath(path) {
  return typeof path === "string" && path.startsWith("/") && !path.startsWith("//") && !path.includes("://");
}

export function setOAuthReturn(res, key, path) {
  if (!isSafeReturnPath(path)) return;
  appendSetCookie(res, buildCookie(RETURN_COOKIE_PREFIX + key, path, { maxAge: 600 }));
}

export function consumeOAuthReturn(req, res, key, fallback) {
  const cookies = parseCookies(req);
  const path = cookies[RETURN_COOKIE_PREFIX + key];
  appendSetCookie(res, clearCookie(RETURN_COOKIE_PREFIX + key));
