...
export async function sendChannelMessage(channelId, content) {
  ...
  if (!res.ok) throw new Error(`Could not send channel message: ${res.status}`);
}

export async function addRoleToUser(discordUserId, roleId) {
...
export async function addDiscordRole(discordUserId) {
  return addRoleToUser(discordUserId, process.env.DISCORD_ROLE_ID_AFVENTER);
}
